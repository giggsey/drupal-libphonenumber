(function ($) {
  ...
  Drupal.PhoneNumber.checkDefault = function(e) {
    ...
    $('.phone-settings .form-checkboxes').find('.form-checkbox').each(function() {
  ...
  Drupal.behaviors.PhoneNumber = {
    attach: function(context) {
      ...
      $('#edit-instance-settings-default-country, .phone-settings .form-checkboxes').bind('change', Drupal.PhoneNumber.checkDefault);
      ...
      var form = '<div class="form-item container-inline">'
               ...
               + '  <a class="phone-check" style="margin-left: 1em;" href="javascript://">' + Drupal.t('Select all') + '</a>'
               + '</div>';

      $('.phone-settings .form-checkboxes', context).before(form);
      $('input.phone-filter').bind('keyup', Drupal.PhoneNumber.filter);
      $('a.phone-check').bind('click', Drupal.PhoneNumber.checkall);
    }
  };
})(jQuery);
